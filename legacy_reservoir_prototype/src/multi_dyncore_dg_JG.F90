...
! CMC_PRECON has length CMC_PRECON(NCOLCMC*IGOT_CMC_PRECON) 
    ...
    SUBROUTINE ASSEMB_FORCE_CTY( state, &
      ...
      REAL, DIMENSION ( :, :, : ), allocatable :: RESID, DIFF_FOR_BETWEEN_U, DIFF_FOR_BETWEEN_V, &
           DIFF_FOR_BETWEEN_W, MAT_ELE, RHS_DIFF_U
      ...
      allocate( rhs_diff_u( ndim_vel, nphase, u_nloc ) ) ; rhs_diff_u = 0.
      ...
      Loop_Elements: DO ELE = 1, TOTELE ! Volume integral
         ...
         DO CV_ILOC = 1, CV_NLOC
            ...
            DO IPHASE=1,NPHASE
               ...
               ENDIF
               DO IDIM = 1, NDIM_VEL
                  LOC_U_SOURCE_CV( IDIM, IPHASE, CV_ILOC) = &
                       U_SOURCE_CV( CV_INOD + (IDIM-1) * CV_NONODS + (IPHASE-1) * NDIM_VEL * CV_NONODS)
               END DO
            END DO
         END DO
         ...
         Loop_ilev_DGNods1: do ilev = 1, nlev
            Loop_DGNods1: do u_iloc = 1 + ( ilev - 1 ) * u_nloc2, ilev * u_nloc2
               ...
               Loop_DGNods2: DO U_JLOC = 1 +(ILEV-1)*U_NLOC2, ILEV*U_NLOC2
                  ...
                  Conditional_Block_Diag: IF(.NOT.JUST_BL_DIAG_MAT) THEN
                     Conditional_StressForm: IF(STRESS_FORM) THEN
                        DO IPHASE = 1, NPHASE
                           DO IDIM = 1, NDIM_VEL 
                              DO JDIM = 1, NDIM_VEL 
