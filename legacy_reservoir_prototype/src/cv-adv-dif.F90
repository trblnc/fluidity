...
!        ewrite(3,*)'ndim=',ndim 
 ...
 !     CALL ONVDLIMsqrt( TOTELE, &
 !          TDLIM, TDCEN, INCOME, PELE, PELEOT, &
 !          ETDNEW, TDMIN, TDMAX, .true., NOLIMI )
...
! If FAST_AND_SIMP use a simple mean to calculate the between element diffusion.
      ...
      REAL, DIMENSION( NDIM,NDIM, NPHASE,SBCVNGI ), intent( out ) :: UDIFF_SUF_STAB
...
! Calculate added stabilization diffusion DIFF_ADD_STAB( SGI,IDIM,IPHASE )
       ...
       *0.5*(DIFF_FOR_BETWEEN_U_ELE(IPHASE,U_ILOC)+DIFF_FOR_BETWEEN_U_ELE2(IPHASE,U_ILOC2))
              ...
              DO IDIM=1,NDIM
                 UDIFF_SUF_STAB(IDIM,IDIM,IPHASE,:)=DIFF_ADD_STAB( : )
              END DO
           END DO
        ELSE
           DO IPHASE=1,NPHASE
...
! the rest is for the diffusion tensor. 
      REAL, DIMENSION( NDIM,  NDIM,NDIM ), intent( in ) :: DIFF_GI_ADDED
      REAL, DIMENSION( CV_NLOC, NPHASE, TOTELE ), intent( in ) :: &
             DUX_ELE,DUY_ELE,DUZ_ELE, DUOLDX_ELE,DUOLDY_ELE,DUOLDZ_ELE,  &
             DVX_ELE,DVY_ELE,DVZ_ELE, DVOLDX_ELE,DVOLDY_ELE,DVOLDZ_ELE,  &
             DWX_ELE,DWY_ELE,DWZ_ELE, DWOLDX_ELE,DWOLDY_ELE,DWOLDZ_ELE
         ...
         IF(SIMPLE_DIFF_CALC) THEN ! The simplest method we can think of... 
            ...
            IF(STRESS_FORM) THEN
               ...
               DIFF_COEF_DIVDX(2)=8.*( SNORMXN(SGI)**2*DIFF_GI(2,1) &
                                   ...
                                   +SNORMZN(SGI)**2*DIFF_GI(2,3)+DIFF_GI_ADDED(1,1, 2) ) /HDC

               DIFF_COEF_DIVDX(3)=8.*( SNORMXN(SGI)**2*DIFF_GI(3,1) &
                                      +SNORMYN(SGI)**2*DIFF_GI(3,2)   &
                                   +2.*SNORMZN(SGI)**2*DIFF_GI(3,3)+DIFF_GI_ADDED(1,1, 3) ) /HDC
            ELSE
               COEF=&
     ...
     +SNORMZN(SGI)*(DIFF_GI(3,1)*SNORMXN(SGI)+ DIFF_GI(3,2)*SNORMYN(SGI)+DIFF_GI(3,3)*SNORMZN(SGI))
               ...
               DIFF_COEF_DIVDX(2)=8.*( COEF + DIFF_GI_ADDED(1,1, 2) ) /HDC
               DIFF_COEF_DIVDX(3)=8.*( COEF + DIFF_GI_ADDED(1,1, 3) ) /HDC
            ENDIF
...
! ENDOF IF(STRESS_FORM) THEN...
     ...
     +SNORMZN(SGI)*(DIFF_GI(3,1)*SNORMXN(SGI)+ DIFF_GI(3,2)*SNORMYN(SGI)+DIFF_GI(3,3)*SNORMZN(SGI))
               ...
               DIFF_STAND_DIVDX_V=8.*( COEF + DIFF_GI_ADDED(1,1, 2) ) /HDC
               DIFF_STAND_DIVDX_W=8.*( COEF + DIFF_GI_ADDED(1,1, 3) ) /HDC
...
! W:
         ...
         ! This is the minimum diffusion...
                  ...
                  DIFF_STAND_DIVDX2_V=8.*( SNORMXN(SGI)**2*DIFF_GI2(2,1) &
                                   ...
                                   +SNORMZN(SGI)**2*DIFF_GI2(2,3)+DIFF_GI_ADDED(1,1, 2) ) /HDC

                  DIFF_STAND_DIVDX2_W=8.*( SNORMXN(SGI)**2*DIFF_GI2(3,1) &
                                      +SNORMYN(SGI)**2*DIFF_GI2(3,2)   &
                                   +2.*SNORMZN(SGI)**2*DIFF_GI2(3,3)+DIFF_GI_ADDED(1,1, 3) ) /HDC
! ENDOF IF(STRESS_FORM) THEN...
               ELSE
                  COEF=&
     ...
     +SNORMZN(SGI)*(DIFF_GI2(3,1)*SNORMXN(SGI)+ DIFF_GI2(3,2)*SNORMYN(SGI)+DIFF_GI2(3,3)*SNORMZN(SGI))
                  ...
                  DIFF_STAND_DIVDX2_V=8.*( COEF + DIFF_GI_ADDED(1,1, 2) ) /HDC
                  DIFF_STAND_DIVDX2_W=8.*( COEF + DIFF_GI_ADDED(1,1, 3) ) /HDC
! ENDOF IF(STRESS_FORM) THEN ELSE...
               ENDIF
...
! determine stress form of viscocity...
      IMPLICIT NONE
      REAL, DIMENSION( 3, 3  ), intent( inOUT ) :: STRESS_IJ
...
!     endif
    ...
    SUBROUTINE GET_INT_VEL_OVERLAP( NPHASE, NDOTQ, INCOME, NDOTQOLD, INCOMEOLD, &
      ...
      REAL :: UDGI,VDGI,WDGI,UOLDDGI,VOLDDGI,WOLDDGI,  &
           ...
           GEOMTOLDGI_IPHA, W_UPWIND, W_UPWINDOLD, INCOME3, INCOME4, INCOMEOLD3, INCOMEOLD4, &
         ...
         Conditional_ELE2: IF(( ELE2 == 0 ).OR.( ELE2 == ELE)) THEN
            ...
            ELSE IF(IN_ELE_UPWIND==3) THEN ! the best optimal upwind frac.
               ...
               if ( .true. ) then
                  ...
                  !  if ( 0.5 * ( ndotq + ndotq2 ) > 0 ) then ! OUTGOING
                  !     W_UPWIND = ( LIMT - T(CV_NODI_IPHA) ) / TOLFUN( T(CV_NODJ_IPHA) - T(CV_NODI_IPHA) )
                  !  else ! INCOMING
                  !     W_UPWIND = 1. - ( LIMT - T(CV_NODJ_IPHA) ) / TOLFUN( T(CV_NODI_IPHA) - T(CV_NODJ_IPHA) )
                  !  end if
                  W_UPWIND = ( LIMT - T(CV_NODI_IPHA) ) / TOLFUN( T(CV_NODJ_IPHA) - T(CV_NODI_IPHA) )
...
!      IF( NDOTQOLD > 0. ) INCOMEOLD = 0.
    ...
    SUBROUTINE GET_INT_T_DEN( FVT, FVTOLD, FVT2, FVT2OLD, FVD, FVDOLD, LIMD, LIMT, LIMT2, &
      ...
      INTEGER, DIMENSION( (TOTELE+1)*IANISOTROPIC ), intent( in ) :: SMALL_FINDRM
      INTEGER, DIMENSION( NSMALL_COLM*IANISOTROPIC ), intent( in ) :: SMALL_COLM
      REAL, DIMENSION( NSMALL_COLM*NPHASE ), intent( in ) :: TUPWIND_MAT, TOLDUPWIND_MAT, DENUPWIND_MAT, DENOLDUPWIND_MAT
      REAL, DIMENSION( NSMALL_COLM*NPHASE*IGOT_T2 ), intent( in ) :: T2UPWIND_MAT, T2OLDUPWIND_MAT
      ! Local variables
            ...
            IF(UPWIND) THEN
               ...
               ELSE ! Standard DG upwinding...
                  ...
                  DO CV_KLOC = 1, CV_NLOC
                     ...
                     ENDIF
                  END DO
                  ! END OF IF(DOWNWIND_EXTRAP.AND.(courant_or_minus_one_new.GE.0.0)) THEN ELSE ...
               ENDIF
...
! the centre of each CV is: XC_CV, YC_CV, ZC_CV
      ...
      REAL, DIMENSION( NSMALL_COLM * CV_NLOC * IELEMATPSI  ), intent( inout ) :: ELEMATWEI
...
! faster results...
      ...
      INTEGER, intent(in) :: IELEMATPSI
      INTEGER, DIMENSION( NCOLM * IELEMATPSI  ), intent( inout ) :: ELEMATPSI
